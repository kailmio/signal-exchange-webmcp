...
- Active shaping: the participant delegated selection of the card powers to Codex based on the judging criteria; the final product keeps Forge and Focus.
...
- Working demo loop: inspect the live board, recommend Forge or Focus, preview the proposed effect, obtain human approval, play the card, animate the shared state change, and retain an undoable activity history.
...
- Core powers locked: Forge and Focus.
...
- Adopted changes: visible WebMCP preview/commit evidence in the demo path, explicit reset/undo semantics, manual/agent result parity, observable preview stability, sub-one-second non-blocking motion, a 1280×800 no-scroll target, and exact keyboard behavior.
...
- Submission wow moment locked: a real agent previews an exact Forge change, the person approves it, Focus identifies the next move, and undo visibly restores the prior board.
...
- Frontend App Builder required a visual concept before code. Generated and accepted a complete ready-state concept, then extracted the palette, typography, layout, component, icon, copy, and responsive rules into `docs/design/design-system.md`.
...
- Implemented typed board state, stable sample data, deterministic Forge/Focus power engine, recommendation, preview-safe command service, synchronous external store, versioned persistence, accessible components, and the six WebMCP tool registrations.
- Verification: 11 Vitest tests passed; strict TypeScript/Vite production build passed; the in-app browser discovered all six WebMCP tools; agent-driven inspect, list, Forge preview/commit, Focus preview/commit, and undo succeeded against live UI state.
...
- Fixed the recommendation progression so a clean board recommends Forge and a forged board recommends Focus.
- Verified the complete manual Forge → Focus → undo flow with real button interactions. Verified custom-mission empty validation and neutral starter cards.
- Verified committed state survives refresh while preview state does not; the restored-state notice is visible.
- The local and durable public builds both expose all six WebMCP tools in the in-app browser. The public smoke test completed inspect, Forge preview, exact commit, Focus, and undo.
...
- Repeated the complete public WebMCP path: reset → inspect → recommend Forge → Forge preview/commit → Focus preview/commit → replay rejection → refresh → undo → reset.
...
- A replay of a consumed preview token returned `PREVIEW_NOT_FOUND` and left the committed board intact. Automated tests separately cover stale, expired, missing, and mismatched guards.
...
## Build checkpoint 5 — Two-power video and submission handoff
