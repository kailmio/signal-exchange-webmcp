...
describe("WebMCP exchange adapter", () => {
  it("registers ten coherent tools with structured results and cleanup", async () => { const captured: Captured[] = []; const modelContext = { registerTool: async (tool: WebMCP.ModelContextTool, options?: WebMCP.ModelContextRegisterToolOptions) => { captured.push({ tool, signal: options?.signal }); } } as unknown as WebMCP.ModelContext; installDocument(modelContext); const store = createStore(createInitialState()); const unregister = await registerWebMcpTools(store, new CommandService(store, { now: () => 1_000, id: () => "id" })); expect(captured.map(({ tool }) => tool.name)).toEqual(["inspect_exchange", "search_data_offers", "compare_data_offers", "preview_data_deal", "commit_data_deal", "undo_last_deal", "load_demo_exchange", "publish_data_offer", "inspect_data_offer", "read_rented_data"]); expect(captured[0].tool.annotations?.readOnlyHint).toBe(true); const result = await captured[0].tool.execute({}, { signal: new AbortController().signal }); expect(result).toMatchObject({ content: [{ type: "text" }], structuredContent: { ok: true } }); expect(store.getState().connection).toBe("ready"); unregister(); expect(captured.every(({ signal }) => signal?.aborted)).toBe(true); });
