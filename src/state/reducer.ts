...
import type { ActivityEntry, AppState, DataOffer, ExchangeContent, Notice, PendingDeal, Recommendation } from "../domain/types";

export type AppAction =
  | { type: "PUBLISH_OFFER"; offer: DataOffer; entry: ActivityEntry }
  ...
  | { type: "SET_SEARCH"; offerIds: string[]; query: string; maxCredits: number; entry: ActivityEntry }
...
const INITIAL_HISTORY: ActivityEntry[] = [
  { id: "activity-market", sequence: 1, origin: "system", kind: "reset", summary: "Marketplace ready", detail: "3 synthetic data offers · publish your own", createdAt: 1 },
...
export function createInitialState(persisted?: PersistedSlice | null): AppState {
  ...
  return { exchange: persisted?.exchange ?? { revision: 0, content }, undoExchange: persisted?.undoExchange ?? null, visibleOfferIds: content.offers.map((offer) => offer.id), searchQuery: content.brief.query, searchMaxCredits: content.brief.budgetCredits, recommendation: null, preview: null, approvedPreviewToken: null, history: persisted?.history?.slice(-20) ?? INITIAL_HISTORY, connection: "checking", persistence: "available", committing: false, notice: persisted ? { tone: "info", text: "Saved demo state restored. Pending deal previews are never restored." } : null };
}
...
export function reducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    case "PUBLISH_OFFER": return { ...state, exchange: { revision: state.exchange.revision + 1, content: { ...state.exchange.content, offers: [...state.exchange.content.offers, action.offer] } }, undoExchange: state.undoExchange ? { ...state.undoExchange, offers: [...state.undoExchange.offers, action.offer] } : null, visibleOfferIds: [...state.exchange.content.offers.map((offer) => offer.id), action.offer.id], searchQuery: "", searchMaxCredits: 100, recommendation: null, preview: null, approvedPreviewToken: null, history: append(state.history, action.entry), notice: { tone: "success", text: `${action.offer.title} published in this local exchange. People and agents can now find it.` } };
    ...
    case "SET_SEARCH": return { ...state, visibleOfferIds: action.offerIds, searchQuery: action.query, searchMaxCredits: action.maxCredits, recommendation: null, preview: null, approvedPreviewToken: null, history: append(state.history, action.entry), notice: { tone: "info", text: `${action.offerIds.length} matching offers are visible.` } };
    case "SET_RECOMMENDATION": return { ...state, recommendation: action.recommendation, history: append(state.history, action.entry), notice: { tone: "info", text: `${action.entry.origin === "agent" ? "Agent" : "Manual"} comparison complete. Review the recommended offer.` } };
